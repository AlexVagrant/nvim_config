...
**单词边界** — 搜独立的标识符而不碰子串：
```
/\<for\>       " 只匹配独立的 for，不匹配 foreach、before
/\<for          " 以 for 开头的单词
/for\>          " 以 for 结尾的单词
```

`*` / `#` 快捷键自带单词边界。想搜光标下词的部分匹配用 `g*` / `g#`。

**关闭正则** — 搜字面文本，`.` `(` `*` 全当普通字符：
```
/\Vfunction(args)   " 精确匹配，括号就是括号
```

`\v` 反过来——默认开 very magic，除了少量元字符外都当正则。
...
建议 `set ignorecase smartcase`：纯小写搜索忽略大小写，带大写自动区分。

**搜索选中文本**：可视模式选中后 `y` 复制，按 `/` 再 `Ctrl-R "` 粘贴到搜索框。
